...
| Panels | Multi-panel without bold panel letters (`a`, `b`, ...) at the upper-left; descriptions not placed above the axes frame line as panel captions; quiver keys not top-right aligned to the caption baseline; inconsistent panel styling |
...
| Panel label clips | Use `ax.text(..., transform=ax.transAxes, clip_on=False)` above the axes; increase top margin |
| Quiver key overlaps caption | Move to top-right on the caption baseline: `quiverkey(Q, 1.0, 1.04, ..., labelpos="W")` |
